...
struct clk *my_clk_get(struct device *dev, const char *id)
{
	struct clk *p;
	...
	printk("my_clk_get before spin_lock\n");
	spin_lock(&clocks_lock);
	spin_unlock(&clocks_lock);
	printk("my_clk_get after spin_lock\n");
	return clk;
...
static int ohci_hcd_s5pc110_drv_probe(struct platform_device *pdev)
{
  ...
  printk("ohci_hcd_s5pc110_drv_probe start\n");
  ...
  printk("getting clock %d\n",to_platform_device(&pdev->dev)->id);
// 	usb_clk = my_clk_get(&pdev->dev, "usb-host");
  printk("just after clk_get\n");
//   return -ENODEV;
